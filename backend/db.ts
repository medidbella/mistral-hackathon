...
export function storeApproval(db: sqlite.Database, currentDate: string, approvalDuration: number,  userId: string)
{
    ...
    if (!existingUser) {
        ...
        db.prepare(`
            INSERT INTO user_stats (user_id, approvals_today, total_duration, last_upload)
            VALUES (?, 1, ?, ?)
        `).run(userId, approvalDuration, currentDate);
    } else {
        ...
        if (lastUploadDate === currentDate) {
            ...
            db.prepare(`
                ...
                SET approvals_today = approvals_today + 1,
                    ...
                    last_upload = ?
                WHERE user_id = ?
            `).run(approvalDuration, currentDate, userId);
        } else {
...
export function getUserStatus(db: sqlite.Database, userId: string): UserStatusResult {
    const user = db.prepare(
        `SELECT approvals_today, total_duration, last_upload FROM user_stats WHERE user_id = ?`
    ).get(userId) as Pick<UserStats, 'approvals_today' | 'total_duration' | 'last_upload'> | undefined;
